CREATE OR REPLACE PROCEDURE UAT_ADS.DWNSTRM_SALESFORCE.SALESFORCE_CONTACT_UPDATE_PARALLEL(
...
def main(session: snowpark.Session, SALESFORCE_UPDATE_FROM_APP):
    ...
    MAX_WORKERS = 10   # run 10 API calls in parallel (each 1 record wrapped in array)
    ...
    def send_record(sf_contact_listing):
        ...
        if contact_data_update:
            # ✅ Safe handling for string or dict
    ...
    return f"Processed {len(results)} records with {MAX_WORKERS} parallel API workers"
$$;
