...
-- Take the backup of this procedure before compiling the new one: PROD_ADS.DWNSTRM_SALESFORCE.SALESFORCE_CONTACT_UPDATE



CREATE OR REPLACE PROCEDURE PROD_ADS.DWNSTRM_SALESFORCE.SALESFORCE_CONTACT_UPDATE(
...
def main(session: snowpark.Session, SALESFORCE_UPDATE_FROM_APP):
    ...
    MAX_WORKERS = 15   # number of parallel API calls
    # -----------------------------------------------------------------
    ...
    url_query = f"""SELECT url AS URL
                    FROM PROD_ADS.DWNSTRM_SALESFORCE.SALESFORCE_PARAM_API_ENDPOINTS
